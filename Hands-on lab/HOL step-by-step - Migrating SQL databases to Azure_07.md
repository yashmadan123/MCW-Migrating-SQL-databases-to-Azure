...
1. Finally, in the **App Service** box, select your subscription, expand the hands-on-lab-<inject key="DeploymentID/<inject key="Suffix" />" /> resource group, and select the `wwi-web-UNIQUEID` Web App.

    ![In the Publish dialog, The wwi-web-UNIQUEID Web App is selected and highlighted under the hands-on-lab-<inject key="DeploymentID/<inject key="Suffix" />" /> resource group.](media/vs-publish-web-app-service.png "Publish API App to Azure")
...
2. Select the hands-on-lab-<inject key="DeploymentID/<inject key="Suffix" />" /> resource group from the list.

   ![Resource groups is selected in the Azure navigation pane, and the "hands-on-lab-<inject key="DeploymentID/<inject key="Suffix" />" />" resource group is highlighted.](./media/resource-groups.png "Resource groups list")

3. In the list of resources for your resource group, select the **hands-on-lab-<inject key="DeploymentID/<inject key="Suffix" />" />** resource group and then select the **wwi-web-UNIQUEID** App Service from the list of resources.
