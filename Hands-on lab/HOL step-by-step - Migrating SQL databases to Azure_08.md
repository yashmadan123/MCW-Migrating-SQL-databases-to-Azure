...
4. On the Network Feature Status dialog, enter the following:
  ...
  > **Note**: If you are not able to select any existing subnet then follow the below steps.
   - Select the create new subnet option and enter name as WebappsubnetSUFFIX. Select Virtual Network address block from the drop down list. In the subnet address block enter new address block 10.0.xx.0/24 for the subnet, Make sure it is not overlapping other subnet's address.
  > **Note**: If the address space is overlapping with other subnets you may need to changes the address space.

   ![The values specified above are entered into the Network Feature Status dialog.](media/newvnet2.png "Network Feature Status configuration")
