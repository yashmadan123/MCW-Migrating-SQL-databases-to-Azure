...
## Task 0: Connect to the SQL Server VM

In this task, you perform some configuration on the SQL Server 2008 R2 instance to configure it for migration, for which you will connect to using Remote Desktop client

1. Open Start-> Remote Desktop. For the host address, copy and enter the `SQL Server VM DNS Name` from the **Environment Details** tab in the guide
...
1. On the Sql2008-[SUFFIX] VM, open **Microsoft SQL Server Management Studio** by entering "sql server" into the search bar in the Windows Start menu.
...
1. Once connected, expand **Databases** under SQL2008-[SUFFIX] in the Object Explorer, and then right-click the **WideWorldImporters** database. In the context menu, select **Tasks** and then **Back Up**.
...
1. Using the command prompt (cmd.exe) from teh start menu, enter the following command to extract the public IP address of the SQL SERVER VM 

    ```Cmd
    ping sqlvmdns323163.centralus.cloudapp.azure.com
...
5. On the Migration Wizard **Select source** tab, enter the following:
   ...
   - **Source SQL Server instance name**: Enter the IP address of your Sql2008-[SUFFIX] VM that you copied in the previous task. For example, `40.65.112.26`.
...
11. On the Migration Wizard **Configure migration settings** tab, enter the following configuration:
    ...
    - **WideWorldImporters**: Enter **Target Database name** WideWorldImporters[SUFFIX].
...
4. Return to the SQL Server Management Studio Window (SSMS) and select **New Query** from the toolbar.
...
1. Within the **Environment Details** tab, locate and copy the value of the `SQL MI FQDN` property.

1. Return to SSMS on your Lab VM, and then select **Connect** and **Database Engine** from the Object Explorer menu.
...
1. The SQL MI connection appears below the SQL2008-[SUFFIX] connection. Expand Databases under the SQL MI connection and select the `WideWorldImporters[SUFFIX]` database.
...
1. With the `WideWorldImporters[SUFFIX]` database selected, select **New Query** on the SSMS toolbar to open a new query window.
